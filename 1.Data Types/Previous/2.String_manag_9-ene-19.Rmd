...
#f) ( 4,6,3,4,6,3, ··· ,4,6,3,4 ) donde hay 11 ocurrencias del número 4, 10 ocurrencias del número 6 y 10 ocurrencias del número 3
v5<-c(v4,4)
...
4.DETEC MATCHES(stringr,htmlwidgets PACKAGE)
4.1.str_view (from "htmlwidgets" library)
```{r}
...
4.2.TRUe-false str_detect (stringr PACKAGE)
```{r}
...
#3. remove accents
  data1$newcol<-chartr("áéíóú", "aeiou",data1$Proceso_afectado) 
```


#4.4. REPLACE SUBTRINGS BY POSITION
